...
def run_main(TestSize, BucketSize):
    ...
    errors = [
        PredictandError(TrainIms, TrainLabels, TestIms[i:(i+BucketSize)], TestLabels[i:(i+BucketSize)])
...
def parse_args():
    ...
    assert (args.test_size == default_test_size or args.test_size >= default_bucket_size), \
