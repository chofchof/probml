...
One way to reduce such overfitting is to <u>prevent the weights from becoming so large</u>. We can do this by using a <u>zero-mean Gaussian prior</u>, $p(w)=\mathcal{N}(w|0,\lambda^{-1}I)$, and then using <u>MAP estimation</u> (see $\S$4.5.3).
...
where $A\otimes B$ is the Kronecker product (see $\S$7.2.5).
...
$$
\begin{align*}
g(W) &= -\sum_n (\mu_n-y_n)\otimes x_n \tag{10.77} \\
...
3. The **nearest class mean classifier** is simple and can easily handle the long tail.
   $$
   f(x) = \arg\min_c\|\phi(x)-\mu_c\|_2^2 \tag{10.98}
   $$
...
> $$
> where $\theta^s\sim p(\theta|\mathcal{D})$ is a posterior sample with the priors $w,b\sim\mathcal{N}(0,10^2)$ and $S=1,000$ (see $\S$10.5).
>
...
$$
p(y|x;\theta) = \pi\operatorname{Ber}(y|\tfrac{1}{2}) + (1-\pi)\operatorname{Ber}(y|\sigma(w^Tx)) \tag{10.99}
$$
...
- The **tempered logarithm** $\log_t\colon\mathbb{R}_+\to\mathbb{R}$ for $0\leq t<1$, is defined by
  $$
  \log_t x\equiv\dfrac{1}{1-t}(x^{1-t}-1) \tag{10.103}
  $$
...
- The **tempered exponential** $\exp_t\colon(-\infty,\frac{1}{t-1})\to\mathbb{R}_+$ for $t>1$, is defined by
  $$
  \exp_t x\equiv \bigl[1+(1-t)x\bigr]^{\frac{1}{1-t}} \tag{10.106}
  $$
...
$$
\begin{align*}
\mathcal{L}(y,\hat y) &= \sum_c \Bigl[ y_c(\log_{t_1}y_c-\log_{t_1}\hat y_c) - \frac{1}{2-t_1}(y_c^{2-t_1}-\hat y_c^{2-t_1}) \Bigr] \tag{10.101} \\
&= \sum_c \Bigl[ \frac{y_c}{1-t_1}(y_c^{1-t_1}-\hat y_c^{1-t_1}) - \frac{1}{2-t_1}(y_c^{2-t_1}-\hat y_c^{2-t_1}) \Bigr] \\
&= - \log_{t_1}\hat y_c - \frac{1}{2-t_1}\Bigl(1-\sum_{c'} \hat y_{c'}^{2-t_1}\Bigr) \quad\text{if $y$ one-hot with $y_c=1$} \tag{10.102}
\end{align*}
$$
...
$$
\lim_{t_1\to1}\mathcal{L}(y,\hat y) = \mathbb{H}(y,\hat y) = -\sum_c y_c\log\hat y_c \tag{10.100}
$$
...
$$
\begin{align*}
\hat y_c = S(a)_c &\equiv \frac{\exp(a_c)}{\sum_{c'}\exp(a_{c'})} = \exp\Bigl[ a_c-\log\sum_{c'}\exp(a_c')\Bigr] \tag{10.104} \\
...
$$
\sum_c\exp_t(a_c-\lambda) = 1 \tag{10.107}
$$
...
However, actually performing this computation is usually intractible, except for simple special cases. We therefore need to <u>approximate the posterior</u> $p(\theta|\mathcal{D})$ (see $\S$4.6.8).
...
$$
where $\hat w_\text{map}$ is the MAP estimate and $H_{\hat w_\text{map}}$ is the Hessian computed at $\hat w_\text{map}$ (see $\S$4.6.8.2).
...
$$
p(y|x;\mathcal{D}) = \int p(y|x;w)p(w|\mathcal{D})\,dw \tag{10.108}
$$
...
$$
p(y=1|x;\mathcal{D}) \approx \frac{1}{S}\sum_s\sigma(w_s^Tx) \tag{10.110}
$$
![](figure_10.14_cd.png)
...
Figure 10.15 (d) shows that the probit approximation gives very similar results to the Monte Carlo approximation. Here, the Laplace approximation $p(w|\mathcal{D})\approx\mathcal{N}(w|\hat w_\text{map},H_{\hat w_\text{map}}^{-1})$ is used.
...
>   &\approx \int\Phi(\lambda a)\mathcal{N}(a|m,v)\,da = \Phi\Bigl[\frac{m}{(\lambda^{-2}+v)^{\frac{1}{2}}}\Bigr] \approx \sigma(k(v)m) \tag{10.111}
>   \end{align*}
>   $$
