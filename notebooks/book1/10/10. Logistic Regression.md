...
> Logistic regression for classifying if an <u>Iris flower</u> is `setosa` ($y=0$) or `versicolor` ($y=1$) using a <u>single input feature</u> $x$ corresponding to <u>sepal length</u>. Labeled points have been (vertically) jittered to avoid overlapping too much. Vertical line is the <u>decision boundary</u>, $x^*=-b/w$.
>
> We use a Bayesian approach (MCMC) to approximate the <u>posterior</u> $p(\theta|\mathcal{D})$. Given this, we can approximate the <u>posterior predictive distribution</u> using a Monte Carlo approximation:
> $$
> p(y=1|x;\mathcal{D}) \approx \frac{1}{S}\sum_s p(y=1|x;\theta^s) \tag{4.205}
> $$
> where $\theta^s\sim p(\theta|\mathcal{D})$ is a posterior sample.
>
> The **left plot**, Figure 4.20 (b) shows the mean and 94% highest density interval of the posterior predictive distribution, and the **right plot**, Figure 10.10 (a) shows the posterior distribution with outliers (denoted by $\times$).
...
$$
In this case, $\theta=(w,\pi)$. This approach, of using a mixture model for the observation model to make it robust, can be applied to many different models, e.g., DNNs.

We can fit this model using standard method, such as SGD or Bayesian inference methods such as <u>MCMC</u>.

![](figure_10.10.png)

> Figure 10.10: (a) Logistic regression on the Iris data with outliers. (b) Same as (a) but using <u>robust model</u>, with a mixture likelihood. Here, the 94% highest density interval of the parameter $\pi$ is (0.152, 0.446) and the mean is 0.302.
...
> **Reference**: E. Amid, et al., *Two-temperature logistic regression based on the Tsallis divergence*, AISTATS 2019.
...
>
> Convex losses have been shown to be <u>prone to outliers</u> as the loss of each individual example <u>unboundedly increases</u> as a function of the activations. These outliers may correspond to (a) extreme examples that <u>lead to large gradients</u>, or (b) misclassified training examples that are <u>located far away from the classification boundary</u>.

![](https://1.bp.blogspot.com/-eu1Ucf8aNzM/XWQMldOSkxI/AAAAAAAAEjM/RH91dqa8zx88oGuS9QaHaqmzf90QBvJHgCLcBGAs/s1600/image3.gif)

This can be overcome by replacing the usual cross entropy loss with a tempered version, that uses a temperature parameter $t_1\in[0,1)$ to ensure <u>the loss from outliers is bounded</u>. We define the **tempered cross entropy loss**:
$$
\begin{align*}
...
\end{align*}
$$
Here, the **tempered log function** $\log_t\colon\mathbb{R}_+\to\mathbb{R}$ for $t\geq 0$,
$$
\log_t(x)\equiv\dfrac{1}{1-t}(x^{1-t}-1) \tag{10.103}
$$
is <u>monotonically increasing and concave</u>, and reduces to the standard (natural) logarithm as $t\to 1$.

Similarly, tempered cross entropy reduces to standard cross entropy,
$$
\mathcal{L}(y,\hat y) = \mathbb{H}(y,\hat y) = -\sum_c y_c\log\hat y_c \tag{10.100}
$$
when $t=1$, since $\sum_c y_c=\sum_c\hat y_c=1$.

However, the <u>tempered log function is bounded below</u> by $\frac{-1}{1-t}$ for $t\in[0,1)$, and hence the <u>cross entropy loss is bounded above</u> (see Figure 10.11 (a)).

![](https://1.bp.blogspot.com/-MSsW9QUCyXM/XWQNInb2ztI/AAAAAAAAEjc/_yu48eAnTjMWQtUbjNqhOlqurgapjSIzACLcBGAs/s1600/Screenshot%2B2019-08-26%2Bat%2B9.39.48%2BAM.png)

> Figure 10.11: (a) Illustration of logistic and tempered logistic loss with $t_1=0.8$. (b) Illustration of sigmoid and tempered sigmoid transfer function with $t_2=2.0$. https://ai.googleblog.com/2019/08/bi-tempered-logistic-loss-for-training.html
...
>
> Another issue arises due to the <u>exponentially decaying tail</u> of the softmax function. In the presence of mislabeled training examples <u>near the classification boundary</u>, the short tail of the softmax probabilities enforces the classifier to stretch the decision boundary towards the noisy training examples. In contrast, <u>heavy-tailed alternatives</u> for the softmax probabilities have been shown to significantly improve the robustness of the loss to these examples.

![](https://1.bp.blogspot.com/-bl02HTabC5s/XWQM1ZIv4VI/AAAAAAAAEjQ/2iewcJCTYnA7w9_eV2wUezUGi6onpNoDACLcBGAs/s1600/image6.gif)

This need to use a transfer function (from activations $\mathbb{R}^C$ to probabilities $[0,1]^C$) that has <u>heavier tails than the softmax</u>, which is based on the exponential.
...
$$
where the **tempered exponential function** $\exp_t\colon\mathbb{R}\to\mathbb{R}_+$ for $t\geq 0$, 
$$
\exp_t(x)\equiv \bigl[1+(1-t)x\bigr]_+^{\frac{1}{1-t}} \quad\text{and}\quad [x]_+\equiv\max\{x,0\} \tag{10.106}
$$
reduces to the standard exponential function as $t\to1$. Figure 10.11 (b) shows that <u>the tempered softmax has heavier tails</u>, as desired.



#### All that remains is a way to compute $\lambda_t(a)\in\mathbb{R}$.

Suppose $t>1$. Since $\sum_c\hat y_c=1$, this must satisfy the fixed point equation
$$
Z_t(a-\lambda\vec{1}) \equiv \sum_c\exp_t(a_c-\lambda)=1 \tag{10.107}
$$
and $\lim_{t\to1}\lambda_t(a)=\log\sum_c\exp(a_c)$, where $Z_t(a)\equiv\sum_c\exp_t(a_c)$.

Note that $\lambda_t(a)$ does not have a closed form solution in general if $t\neq 1$, and must be calculated numerically. We can find $\lambda_t(a)$ using binary search or by using the iterative procedure in Algorithm 3.

<img src="algorithm_10.3.png" style="zoom: 50%;" />

> **Proof**. We may assume that $\mu=0$ for simple notations. For logits $a\in\mathbb{R}^C$ and a temperature $t>1$, let
> $$
> \tilde a_{k+1}\equiv Z_t(\tilde a_k)^{1-t}a \quad\text{and}\quad \tilde a_0=a
> $$
> Then $\tilde a_k$ is a strictly decreasing sequence (by induction) and bounded below so that the limit $\tilde a\equiv\lim_{k\to\infty}\tilde a_k$ exists. We will show that
> $$
> \lambda_t(a) \equiv - \log_t\frac{1}{Z_t(\tilde a)} \quad\text{satisfies}\quad
> \exp_t(a_c-\lambda_t(a)) = \frac{\exp_t(\tilde a_c)}{Z_t(\tilde a)}
> $$
> Then (eq. 10.107) follows immediately, since $Z_t(\tilde a)=\sum_c\exp_t(\tilde a_c)$. It is also trivial to check that $\lim_{t\to1}\lambda_t(a)=\log\sum_c\exp(a_c)$.
>
> Since $\tilde a_c=\lim_{k\to\infty}\tilde a_{kc}=Z_t(\lim_{k\to\infty}\tilde a_k)^{1-t}a_c=Z_t(\tilde a)^{1-t}a_c$, we have
> $$
> \begin{align*}
> \exp_t(a_c-\lambda_t(a)) &= \exp\Bigl[a_c + \log_t\frac{1}{Z_t(\tilde a)}\Bigr] = \Bigl[1+(1-t)a_c + (1-t)\log_t\frac{1}{Z_t(\tilde a)}\Bigr]_+^{\frac{1}{1-t}} \\
> &= \Bigl[(1-t)a_c + Z_t(\tilde a)^{t-1}\Bigr]_+^{\frac{1}{1-t}}
> = \Bigl[Z_t(\tilde a)^{t-1}\bigl[1+(1-t)\tilde a_c\bigr]\Bigr]_+^{\frac{1}{1-t}}
> = \frac{\exp_t(\tilde a_c)}{Z_t(\tilde a)}
> \end{align*}
> $$
...
Combining the tempered softmax with the tempered cross entropy results in a method called **bi-tempered logistic regression**.
