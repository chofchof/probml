...
> $$
> where $\theta^s\sim p(\theta|\mathcal{D})$ is a posterior sample and $S=1,000$ (see $\S$10.5).
>
...
- The **tempered logarithm** $\log_t\colon\mathbb{R}_+\to\mathbb{R}$ for $0\leq t<1$, is defined by
  ...
  Note that $\log_t 1=0$ and $\log x\leq\log_t x\leq\log_0x=x-1$ for all $t$.
...
- The **tempered exponential** $\exp_t\colon(-\infty,\frac{1}{t-1})\to\mathbb{R}_+$ for $t>1$, is defined by
  ...
  Note that $\exp_t 0 = 1$ and $\exp x\leq\exp_t x$ for all $t$. Moreover, it is the inverse of the tempered logarithm.
...
Since $\sum_c y_c=\sum_c\hat y_c=1$, it reduces to the standard cross entropy:
$$
\lim_{t_1\to1}\mathcal{L}(y,\hat y) = \mathbb{H}(y,\hat y) = -\sum_c y_c\log\hat y_c \tag{10.100}
$$
Notice that the tempered log function is <u>bounded below</u> by $\frac{-1}{1-t}$ for $t\in[0,1)$, and hence the cross entropy loss is <u>bounded above</u> (see Figure 10.11 (a)).

![](figure_10.11.png)

> Figure 10.11: (a) Illustration of logistic and tempered logistic loss with $t_1=0.8$. (b) Illustration of sigmoid and tempered sigmoid transfer function with $t_2=2.0$.
...
#### All that remains is to compute $\lambda_t(a)$.

> **Reference**: [Ami+19] E. Amid, et al., *Robust Bi-Tempered Logistic Loss Based on Bregman Divergences*, NeurIPS 2019.

Suppose $t>1$. Since $\sum_c\hat y_c=1$, the real value $\lambda_t(a)$ must satisfy the fixed point equation
$$
\sum_c\exp_t(a_c-\lambda) = 1 \tag{10.107}
$$
and $\lim_{t\to1}\lambda_t(a)=\log\sum_c\exp(a_c)$.
...
> $$
> where $\mu\equiv\max_c\{a_c\}$ and $Z_t(a)\equiv\sum_c\exp_t(a_c)\in\mathbb{R}$.
>
> Then $\tilde a_k$ is a strictly decreasing sequence (by induction) and bounded below so that the limit $\tilde a\equiv\lim_{k\to\infty}\tilde a_k$ exists. We assert that
> $$
> \lambda_t(a) \equiv - \log_t\frac{1}{Z_t(\tilde a)} + \mu \quad\text{satisfies}\quad
> \exp_t(a_c-\lambda_t(a)) = \frac{\exp_t(\tilde a_c)}{Z_t(\tilde a)} \tag{*}
> $$
...
>
> It remains to show (*). Since $Z_t$ is continuous,
> $$
> \tilde a_c=\Bigl(\lim_{k\to\infty}\tilde a_{k+1}\Bigr)_c=Z_t(\lim_{k\to\infty}\tilde a_k)^{1-t}(a_c-\mu)=Z_t(\tilde a)^{1-t}(a_c-\mu)
> $$
> It follows that
> $$
> \begin{align*}
> \exp_t(a_c-\lambda_t(a)) &= \exp_t\Bigl[(a_c-\mu) + \log_t\frac{1}{Z_t(\tilde a)}\Bigr] \\
> &= \Bigl[1+(1-t)(a_c-\mu) + (1-t)\log_t\frac{1}{Z_t(\tilde a)}\Bigr]_+^{\frac{1}{1-t}} \\
> &= \Bigl[(1-t)(a_c-\mu) + \frac{1}{Z_t(\tilde a)^{1-t}}\Bigr]_+^{\frac{1}{1-t}} \\
> &= \Bigl[\frac{1}{Z_t(\tilde a)^{1-t}}\bigl[1+(1-t)\tilde a_c\bigr]\Bigr]_+^{\frac{1}{1-t}} \\
...
Combining the <u>tempered softmax</u> with the <u>tempered cross entropy</u> results in a method called **bi-tempered logistic regression**.
