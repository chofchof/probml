...
We want to compute the posterior $p(\theta|\mathcal{D})$ in order to capture our uncertainty, not point estimates e.g., MAE or MAP. This can be useful when we have little data, and when choosing the wrong decision may be costly.

Given a likelihood $p(\mathcal{D}|\theta)$ and a prior $p(\theta)$, we can compute the posterior $p(\theta|\mathcal{D})$ using Bayes' rule. However, actually performing this computation is usually intractible, except for simple special cases. We therefore need to approximate the posterior (see $\S$4.6.8).



### 10.5.1 Quadratic (Laplace) approximation

The **Laplace approximation** or a **quadratic approximation** approximates the posterior using a multivariate Gaussian.
$$
p(w|\mathcal{D}) \approx \mathcal{N}(w|\hat w, H_{\hat w}^{-1})
$$
where $\hat w$ is the MAP estimate and $H_{\hat w}$ is the Hessian computed at $\hat w$.
