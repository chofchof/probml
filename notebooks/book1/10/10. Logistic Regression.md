...
$$
p(y|x) = \pi\operatorname{Ber}(y|\tfrac{1}{2}) + (1-\pi)\operatorname{Ber}(y|\sigma(w^Tx)) \tag{10.99}
$$
...
We can fit this model using standard method, such as SGD or Bayesian inference methods such as MCMC.

![](figure_10.10.png)

> Figure 10.10: (a) Logistic regression on the <u>Iris data</u> with outliers (denoted by $\times$). Training points have been (vertically) jittered to avoid overlapping too much. Vertical line is the <u>decision boundary</u>, and its posterior credible interval. (b) Same as (a) but using <u>robust model</u>, with a mixture likelihood.
