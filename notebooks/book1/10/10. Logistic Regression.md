...
> $$
> 
>The line is drawn from the origin in the direction of the MLE (which is at infinity). Notice that
> $$
> w_i(1) / w_i(2) \approx \hat w_\text{mle}(1) / \hat w_\text{mle}(2) \quad\text{for $i=1,2,3,4$}
> $$

![](figure_10.13_cd.png)

> Figure 10.13: (c) Unnormalized log posterior (assuming vague spherical prior).
> $$
> \log p(w|\mathcal{D}) \propto \log p(\mathcal{D}|w) + \log p(w) = \operatorname{LL}(w) + \log \mathcal{N}(w|0,10^2I)
> $$
