...
>
> (b) Negative log-likelihood $\operatorname{NLL}(w)\equiv-\log p(\mathcal{D}|w)$ of binary logistic regression model. The (blue) diagonal line connects the origin to the point, $\hat w_\text{mle}=(9.91, 4.37)$, computed using
>
> ```python
...
If we assume that $p(w|\mathcal{D})$ is a <u>Gaussian</u> $q(w)$, there is a <u>simple yet accurate deterministic</u> approximation, called the **probit approximation**. Set $q(w)=\mathcal{N}(w|\hat w_\text{map},H_{\hat w_\text{map}}^{-1})$, the Laplace approximation.

Denoting $a=w^Tx$ we have $\sigma(w^Tx) = \int_a\delta(a-w^Tx)\sigma(a)\,da$, where $\delta$ is the Dirac delta function. From this we obtain
$$
\int_w\sigma(w^Tx)q(w)\,dw = \int_a\sigma(a)\Biggl[\int_w\delta(a-w^Tx)q(w)\,dw\Biggr]\,da = \int_a\sigma(a)p(a)\,da
$$
where $p(a)=\int_w\delta(a-w^Tx)q(w)\,dw.$ Since $q(w)$ is Gaussian, the marginal distribution $p(a)$ is also Gaussian. We can evaluate the mean and covariance of $p(a)$ so that $p(a)=\mathcal{N}(a|m,v)$ where
$$
\begin{align*}
m \equiv \mathbb{E}_{p(a)}[a] &= \int_a p(a)a\,da = \int_w q(w)\int_a \delta(a-w^Tx)a\,da\,dw \\
&= \int_w q(w)w^Tx\,dw = \mathbb{E}_{q(w)}[w^Tx] = \hat w_\text{map}^Tx \\
v \equiv \mathbb{V}_{p(a)}[a] &= \int_a p(a)\bigl(a^2-\mathbb{E}[a]^2\bigr)\,da = \int_w q(w)\int_a \delta(a-w^Tx)(a^2-\mathbb{E}[a]^2)\,da\,dw \\
&= \int_w q(w)\bigl[(w^Tx)^2-(\hat w_\text{map}^Tx)^2\bigr]\,dw = \mathbb{V}_{q(w)}[w^Tx] = x^T H_{\hat w_\text{map}}^{-1} x
\end{align*}
$$
The key observation is that $\sigma(a)\approx\Phi(\lambda a)$ where $\Phi$ is the Gaussian cdf and $\lambda^2=\pi/8$  (see [Bis06, p.219]). Thus the posterior predictive distribution becomes
$$
\begin{align*}
p(y=1|x;\mathcal{D}) &= \int_w p(y=1|x;w)p(w|\mathcal{D})\,dw
\approx \int_w \sigma(w^Tx)q(w)\,dw = \int_a \sigma(a)p(a)\,da \\
...
In the multi-class case we can use the **generalized probit approximation**:
$$
p(y=c|x;\mathcal{D}) \approx \frac{\exp(\kappa(v_c)m_c)}{\sum_{c'}\exp(\kappa(v_{c'})m_{c'})}
$$
where $\kappa(v_c)=(1+\pi v_c/8)^{-\frac{1}{2}}$. Here, $m_c=\bar m_c^Tx$, and $v_c=x^T V_{c,c}x$. [Gib97, $\S$5.4, https://citeseerx.ist.psu.edu/viewdoc/download?doi=10.1.1.147.1130&rep=rep1&type=pdf]
