...
If we assume that $p(w|\mathcal{D})$ is a <u>Gaussian</u> $q(w)$, there is a <u>simple yet accurate deterministic</u> approximation, called the **probit approximation**.

From the sifting property $\sigma(w^Tx) = \int\delta(a-w^Tx)\sigma(a)\,da$ of the Dirac delta function $\delta$, we obtain
$$
\int\sigma(w^Tx)q(w)\,dw = \int\sigma(a)\Biggl[\int\delta(a-w^Tx)q(w)\,dw\Biggr]\,da = \int\sigma(a)p(a)\,da
$$
where $p(a)=\int\delta(a-w^Tx)q(w)\,dw.$ Since $q(w)$ is Gaussian, the marginal distribution $p(a)$ is also Gaussian. We can evaluate the mean and covariance of $p(a)=\mathcal{N}(a|m,v)$:
$$
\begin{align*}
m \equiv \mathbb{E}[a] &= \int p(a)a\,da = \int q(w)\Biggl[\int\delta(a-w^Tx)a\,da\Biggr]\,dw \\
&= \int q(w)w^Tx\,dw = \mathbb{E}[w^Tx] = \mathbb{E}[w]^Tx \\
v \equiv \mathbb{V}[a] &= \int p(a)a^2\,da-m^2 = \int q(w)\Biggl[\int\delta(a-w^Tx)a^2\,da\Biggr]\,dw-m^2 \\
&= \int q(w)(w^Tx)^2\,dw-\mathbb{E}[w^Tx]^2 = \mathbb{V}[w^Tx] = x^T\mathbb{V}[w] x
\end{align*}
$$
...
Figure 10.15 (d) shows that the probit approximation gives very similar results to the Monte Carlo approximation, where $q(w)=\mathcal{N}(w|\hat w_\text{map},H_{\hat w_\text{map}}^{-1})$ is the Laplace approximation.
