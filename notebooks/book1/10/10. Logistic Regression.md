...
> $$
> where $\theta^s\sim p(\theta|\mathcal{D})$ is a posterior sample with the priors $w,b\sim\mathcal{N}(0,10)$ and $S=1,000$ (see $\S$10.5).
>
...
- The **tempered exponential** $\exp_t\colon(-\infty,\frac{1}{t-1})\to\mathbb{R}_+$ for $t>1$, is defined by
  ...
  Note that $\exp_t 0 = 1$ and $\exp x\leq\exp_t x$. Moreover, $\exp_tx$ is the <u>inverse</u> of $\log_tx$.
...
> $$
> \tilde a_{k+1}\equiv Z_t(\tilde a_k)^{1-t}(a-\mu\vec{1}) \quad\text{and}\quad \tilde a_0=a-\mu\vec{1}
> $$
> where $Z_t(a)\equiv\sum_c\exp_t(a_c)\in\mathbb{R}$ and $\mu\equiv\max_c\{a_c\}$.
>
> We can show that $Z_t(\tilde a_k)$ is strictly decreasing (by induction) and bounded below by 1. Thus the limit $\tilde z\equiv\lim_{k\to\infty}Z_t(\tilde a_k)=Z_t(\lim_{k\to\infty}\tilde a_k)\equiv Z_t(\tilde a)$ exists so that
> $$
> \tilde a_c=\Bigl(\lim_{k\to\infty}\tilde a_{k+1}\Bigr)_c=Z_t(\lim\nolimits_{k\to\infty}\tilde a_k)^{1-t}(a_c-\mu)=\tilde z^{1-t}(a_c-\mu)
> $$
> It follows that $\lambda_t(a) \equiv - \log_t\tilde z^{-1} + \mu$ satisfies
> $$
> \begin{align*}
> \exp_t(a_c-\lambda_t(a)) &= \exp_t\Bigl[(a_c-\mu) + \log_t\tilde z^{-1}\Bigr] \\
> &= \Bigl[1+(1-t)(a_c-\mu) + (1-t)\log_t\tilde z^{-1}\Bigr]^{\frac{1}{1-t}} \\
> &= \Bigl[(1-t)(a_c-\mu) + \tilde z^{-(1-t)}\Bigr]^{\frac{1}{1-t}} \\
> &= \Bigl[\tilde z^{-(1-t)}\bigl[1+(1-t)\tilde a_c\bigr]\Bigr]^{\frac{1}{1-t}} \\
> &= \tilde z^{-1}\exp_t(\tilde a_c)
> \end{align*}
> $$
> Therefore, $\sum_c\exp_t(a_c-\lambda_t(a))=\tilde z^{-1}\sum_c\exp_t(\tilde a_c)=\tilde z^{-1}Z_t(\tilde a)=1$. It is trivial to check that $\lim_{t\to1}\lambda_t(a)=\log\sum_c\exp(a_c)+\mu$.
...
We want to compute the posterior $p(\theta|\mathcal{D})$ in order to <u>capture our uncertainty</u>, not point estimates, MAE or MAP. This can be useful when we have little data, and when choosing the wrong decision may be costly.
...
- **MAP estimate (posterior mode)**: $\hat\theta_\text{map}\equiv\arg\max_\theta p(\theta|\mathcal{D})$
...
$$
p(w|\mathcal{D}) \approx \mathcal{N}(w|\hat w, H_{\hat w}^{-1})
$$
where $\hat w$ is the MAP estimate and $H_{\hat w}$ is the Hessian computed at $\hat w$ (see $\S$4.6.8.2).

![](figure_10.13_ab.png)

> Figure 10.13: (a) Illustration of the data. (b) Log-likelihood for a logistic regression model.
>
> $$
> \operatorname{LL}(w) = \sum_n\bigl[ y_n\log\sigma(w^Tx_n)+(1-y_n)\log(\sigma(-w^Tx_n))\bigr] \tag{10.6}
> $$
> 
>The line is drawn from the origin in the direction of the MLE (which is at infinity). Notice that $w_i(1) / w_i(2) \approx \hat w_\text{mle}(1) / \hat w_\text{mle}(2)$ for $i=1,2,3,4$.
...
$$
\log p(w|\mathcal{D}) = \operatorname{LL}(w) + \log \mathcal{N}(w|0,\sigma^2I) + \text{const}
$$

Note that $\sigma^2$ controls the strength of the prior.

- $\sigma^2\to 0$ implies $\hat w_\text{map}=0$; <u>maximally uncertain predictions</u>, since
  $$
  p(y=1|x)=\sigma(\hat w_\text{map}^Tx)=\sigma(0)=\tfrac{1}{2}
  $$

- $\sigma^2\to\infty$ implies that the prior becomes uninformative and $\hat w_\text{map}=\hat w_\text{mle}$; <u>minimally uncertain predictions</u>.

In Figure 10.12 (d), the shape of the posterior is distorted. The orientation of the blue line shows uncertainty about the magnitude of $w$, and the orientation of the red line shows uncertainty about the decision boundary.



![](figure_10.14_ab.png)

> Figure 10.14: Posterior predictive distribution for a logistic regression model in 2d. (a) Contours of $p(y=1|x;\hat w_\text{map})$. (b) 100 samples from the Laplace approximation $w\sim\mathcal{N}(w|\hat w_\text{map},H_{\hat w_\text{map}}^{-1})\approx p(w|\mathcal{D})$.
...
$$
A simple approach is to first compute a point estimate $\hat w$, such as the MLE or MAP estimate, and then to ignore all posterior uncertainty, by assuming $p(w|\mathcal{D})=\delta(w-\hat w)$ where $\delta$ is the Dirac-delta function. In the case we have
$$
...
If we assume that $p(w|\mathcal{D})$ is a <u>Gaussian</u> distribution, there is a <u>simple yet accurate deterministic</u> approximation, called the **probit approximation**.

The key observation is that $\sigma(a)\approx\Phi(\lambda a)$ where $\Phi$ is the Gaussian cdf and $\lambda^2=\pi/8$.

If we define $a=w^Tx$, we have
$$
p(y=1|x;\mathcal{D}) = \int\Phi(\lambda a)\mathcal{N}(a|m,v)\,da = \Phi\Bigl[\frac{m}{(\lambda^{-2}+v)^{\frac{1}{2}}}\Bigr] \approx \sigma(k(v)m) \tag{10.111}
$$
where $\kappa(v) \equiv \bigl(1+\pi v/8\bigr)^{-\frac{1}{2}}$ and $m=\mathbb{E}[a]=\hat w_\text{map}^Tx$ and $v=\mathbb{V}[a]=x^T H_{\hat w_\text{map}}^{-1} x$ (see [Bis06, p.219]).

Figure 10.15 (d) shows that the probit approximation gives very similar results to the Monte Carlo approximation.
