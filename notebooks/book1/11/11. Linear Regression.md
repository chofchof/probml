...
Suppose we have a dataset of $N$ people in which we record their heights $h_n$, as well as the length of their left legs $l_n$ and right legs $r_n$. Suppose $h_n\sim\mathcal{N}(10,2)$, so the average height is $\bar h=10$ (in unspecified units). Suppose the length of the legs in some fraction $\rho_n\sim \mathcal{U}(0.4,0.5)$ of the height, plus a bit of Gaussian noise, specifically $l_n,r_n\sim\mathcal{N}(\rho_nh_n,0.02)$.

We want to predict the height of a person given measurement of their leg lengths. Since both left and right legs are noisy measurements of the unknown quantity, it is useful to use both of them. So we use linear regression to fit $p(h|l,r)=\mathcal{N}(h|\alpha+\beta_ll+\beta_rr,\sigma^2)$. We use vague priors, $\alpha,\beta_l,\beta_r\sim\mathcal{N}(0,100)$ and $\sigma\sim\operatorname{Expon}(1)$.

Since the average leg length is $\bar l=0.45\bar h=4.5$, we might expect each $\beta$ coefficient to be around $\bar h/\bar l=10/4.5=2.2$. However, the posterior marginals shown in Figure 11.23 tell a different story.
