...
- A more interpretable measure can be computed using the **coefficient of determination**, denoted by $R^2$:
  ...
  We see that $R^2$ measures the <u>variance in the predictions relative to a simple constant prediction</u> of $\hat y_n=\bar y$. Note that $0\leq R^2\leq 1$, where larger values imply a greater reduction in variance (better fit).
