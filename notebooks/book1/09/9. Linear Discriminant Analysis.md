...
The projection from $x\in\mathbb{R}^D$ to $z\in\mathbb{R}^K$ is accomplished by $K\leq C-1$ discriminant functions $z_k=w_k^T x$. If the weight vectors $w_k$ are viewed as the columns of $D\times K$ matrix $W$, then the projection can be written as $z-m=W^T(x-\mu)$.
...
- `clf.intercept_`: of shape `(C,)` with values $\gamma_c$ so that $\mu_c^T\beta_c=(\mu_c-\mu)^T\beta_c+2\mu^T\beta_c=\|m_c-m\|^2+2\mu^T\beta_c$ (eq. 9.7). 
