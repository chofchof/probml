...
Use the weighted sum $\sum_c\pi_c\hat\Sigma_c$ of empirical covariances as a shared covariance $\hat\Sigma$. In particular, when $\pi_c=\frac{N_c}{N}$, we have within-class scatter matrix $\frac{1}{N}S_W$:
$$
\hat\Sigma \equiv \sum_c \frac{N_C}{N} \hat\Sigma_c = \frac{1}{N} \sum_c \sum_{n:y_n=c} (x_n-\hat\mu_c)(x_n-\hat\mu_c)^T \equiv \frac{1}{N} S_W \text{\quad(within-class scatter)} \tag{9.21}
$$
...
$$
where $d$ is the Mahalanobis distance. This is called the **nearest centroid classifier**, or **nearest class mean classifier (NCM)**. We can replace $d$ with any other distance metric.

One simple approach to learn $d$ is to use
$$
...
Let $\mu_c=\frac{1}{N_C}\sum_{n:y_n=c} x_n$ be the class mean and $\mu=\frac{1}{N}\sum_c N_c\mu_c=\frac{1}{N}\sum_n x_n$ be the overall mean. Define the **between-class scatter** $S_B\equiv S_T-S_W$, where
$$
\begin{align*}
...
\end{align*}
$$
The goal is to find a transformation matrix $W$ that maximize the ratio of the between class scatter to the within-class scatter. A simple scalar measure of scatter is the determinant of the scatter matrix
$$
J(W) = \frac{|\tilde S_B|}{|\tilde S_W|} = \frac{|W^TS_BW|}{|W^TS_WW|}
$$
The problem of finding $W$ that maximize $J(W)$ is tricky, though it turns out that the solution is relatively simple. The rows of an optimal $W$are the generalized eigenvectors $w_k$ that correspond to the largest eigenvalues $\lambda_k$ in
$$
S_B w_k = \lambda_k S_W w_k
$$
Note that $W^TS_BW=\operatorname{diag}(\lambda_k)$ and $W^TS_WW=I_K$. In general the solution for $W$ is not unique.
...
clf.fit(X, y)
X_trans = clf.transform(X) # dimension reduction, X @ clf.scalings_
clf.decision_function(X) # discriminant function w/o const (eq. 9.7)
clf.predict(X) # argmax of decision_function(X)
clf.predict_proba(X) # softmax of decision_function(X) except for the binary
                     # classification: (1-p, p), where p=sigmoid(decision_function(X))
...
- `clf.coef_`: of shape `(C,D)`. Each row is $\beta_c=WW^T\mu_c$ (eq. 9.7).
- `clf.intercept_`: of shape `(C,)` with values $\gamma_c$ (eq. 9.7).
